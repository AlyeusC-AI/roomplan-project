...
export default function RoomScopeScreen() {
  ...
  const handleExtraFieldChange = useCallback(
    (areaType: string, areaId: number, fieldId: string, value: string) => {
      const updatedArea = {
  ...
  const fetchRoom = () => {
    setLoading(true);
    api
      .get(`/api/v1/projects/${projectId}/room`)
      .then((res) => {
        const data = res.data;
  ...
  const saveAffectedArea = async (data: any, type: string, roomId: string) => {
    setSaving(true);
    setHasChanges(true);

    try {
      const res = await api.post(
        `/api/v1/projects/${projectId}/room/affected-area`,
        {
          roomId,
  ...
  // Add handleAddCustomEquipment function
  const handleAddCustomEquipment = async () => {
    if (!newCustomEquipment.trim()) return;

    try {
      // const response = await api.patch("/api/v1/organization", {
  ...
  // Update handleDeleteCustomEquipment to use confirmation
  ...
  // Update filteredEquipment to include custom equipment
  const filteredEquipment = useMemo(() => {
    const allEquipment = [...equipmentOptions, ...customEquipment];
    return allEquipment.filter((equipment) =>
      equipment.toLowerCase().includes(equipmentSearch.toLowerCase())
    );
  }, [equipmentSearch, customEquipment]);
      ...
      <View className="flex-1 bg-background">
        ...
        <ScrollView className="flex-1 bg-background">
          <View className="px-5 py-6">
                  ...
                  <View style={styles.dimensionGroup}>
                    <View style={styles.dimensionInput}>
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          handleRoomDetailChange("length", text)
                        }
                    ...
                    <View style={styles.dimensionInput}>
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          handleRoomDetailChange("width", text)
                        }
                        placeholderTextColor="#94a3b8"
                      />
                      <Text style={styles.unitLabel}>ft</Text>
                    ...
                    <View style={styles.dimensionInput}>
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          handleRoomDetailChange("height", text)
                        }
                        placeholderTextColor="#94a3b8"
                      />
                      <Text style={styles.unitLabel}>ft</Text>
                  ...
                  <View style={styles.dimensionGroup}>
                    <View style={styles.dimensionInput}>
                      ...
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          handleRoomDetailChange("doors", text)
                        }
                        placeholderTextColor="#94a3b8"
                      />
                    </View>
                    <View style={styles.dimensionInput}>
                      ...
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          handleRoomDetailChange("windows", text)
                        }
                        placeholderTextColor="#94a3b8"
                      />
                    </View>
            ...
            {(room?.AreaAffected?.filter((a: any) => !a.isDeleted) || [])
              .length > 0 ? (
              <View style={styles.card}>
                <View style={styles.cardHeader}>
                                ...
                                <View key={fieldId} className="mb-4">
                                  ...
                                  <View className="relative">
                                    <TextInput
                                      ...
                                      keyboardType="numeric"
                                      value={
                                        extraFieldValues[fieldId] ?? field.value
                                      }
                                      onChangeText={(text) => {
                                        setExtraFieldValues((prev) => ({
                                          ...prev,
                      ...
                      onPress={() => {
                        if (editingExtraField) {
                          handleUpdateExtraField(
                            activeTab,
                            room.AreaAffected.find(
                              (a: any) => a.type === activeTab
                            )?.id,
