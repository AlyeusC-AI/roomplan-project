...
import { View, Text, StyleSheet, ActivityIndicator, ScrollView, TouchableOpacity, SafeAreaView, Share, Platform, Alert, Modal, FlatList, SectionList, ActionSheetIOS } from 'react-native';
import { Stack, useLocalSearchParams, useRouter } from 'expo-router';
import { Invoice, SavedLineItem } from '@/lib/state/invoices';
import { fetchInvoiceById, emailInvoice, addInvoiceItem } from '@/lib/api/invoices';
import { getSavedLineItems } from '@/lib/api/savedLineItems';
import { StatusBar } from 'expo-status-bar';
import { formatDate } from '@/utils/date';
import { formatCurrency } from '@/utils/formatters';
import { showToast } from '@/utils/toast';
import { X, Mail, Download, Clock, DollarSign, Share2, Plus } from 'lucide-react-native';
...
export default function InvoiceDetailsScreen() {
  ...
  useEffect(() => {
    ...
    const loadInvoice = async () => {
      try {
        const result = await fetchInvoiceById(publicId);
        if (result && 'data' in result && result.data) {
          setInvoice(result.data);
        } else if (result && 'error' in result && result.error) {
          setError(result.error);
          showToast('error', 'Error', `Failed to load invoice: ${result.error}`);
        } else {
        ...
        showToast('error', 'Error', `Failed to load invoice: ${errorMessage}`);
      } finally {
        setLoading(false);
      }
    };
        ...
        <body>
          ...
          <table class="amount-table">
            <tbody>
              <tr class="total-row">
                <td>Total</td>
                <td style="text-align: right;">${formatCurrency(invoice.amount / 100)}</td>
              </tr>
            </tbody>
          </table>
        </body>
  ...
  // Load saved line items when opening the sheet
  const loadSavedLineItems = useCallback(async () => {
  ...
  useEffect(() => {
    if (showSavedItemsSheet) {
      loadSavedLineItems();
    }
  }, [showSavedItemsSheet, loadSavedLineItems]);
  ...
  const groupedItems = useCallback(() => {
    ...
    return categories.map(category => ({
      title: category,
      data: savedLineItems.filter(item => (item.category || 'Uncategorized') === category)
    }));
  }, [savedLineItems]);

  // Handle adding a saved line item to the invoice
  const handleAddSavedLineItem = async (item: SavedLineItem) => {
    if (!invoice || !publicId) return;
    
    setIsAddingItem(true);
    try {
      const result = await addInvoiceItem(publicId, {
      ...
      if (result && 'data' in result && result.data) {
        // Update the local invoice data with the new item
        setInvoice({
          ...invoice,
        ...
        showToast('error', 'Error', `Failed to add item: ${result.error}`);
      }
    } catch (error) {
            ...
            <SafeAreaView style={styles.sheetSafeArea}>
              ...
              {loadingSavedItems ? (
                ...
                <View style={styles.emptyContainer}>
                  ...
                  >
                    <Text style={styles.emptyStateButtonText}>Add Saved Items</Text>
                  </TouchableOpacity>
                </View>
              ) : (
                <>
                    ...
                    renderItem={({ item }) => (
                      ...
                      >
                        <View style={styles.savedItemContent}>
                          ...
                          <Text style={styles.savedItemRate}>{formatCurrency(item.rate)}</Text>
                        </View>
  ...
  const handleSendInvoice = async () => {
    ...
    setIsSending(true);
    try {
      const result = await emailInvoice(publicId);
      
      if (result && 'data' in result && result.data && result.data.success) {
        showToast('success', 'Success', result.data.message);
      } else if (result && 'error' in result && result.error) {
        showToast('error', 'Error', `Failed to send invoice: ${result.error}`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        if (status !== 'granted') {
          Alert.alert('Permission needed', 'Please grant permission to save the invoice');
      ...
      const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
      showToast('error', 'Error', `Failed to download invoice: ${errorMessage}`);
    } finally {
      setIsDownloading(false);
    }
  };
        ...
        <>
          <ScrollView style={styles.content}>
            ...
            <View style={styles.section}>
              ...
              <Text style={styles.amount}>{formatCurrency(invoice.amount / 100)}</Text>
            </View>
                  ...
                  <View key={index} style={styles.lineItem}>
                    ...
                    <View style={styles.lineItemDetails}>
                      <Text style={styles.lineItemQuantity}>
                        {item.quantity} × {formatCurrency(item.rate / 100)}
                      </Text>
