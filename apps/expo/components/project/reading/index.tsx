...
import { TouchableOpacity, Text, View, Alert, ActivityIndicator, Pressable, Image } from "react-native";
...
const RoomReading: React.FC<RoomReadingProps> = ({ room, reading, addReading }) => {
  // Create a type-safe version of the reading with all required properties
  const typedReading = reading as any & {
  ...
  const getExtendedWallImagesForUI = (wallId: string): { key: string; uri: string }[] => {
    if (!reading.RoomReadingImage) return [];
    
    const images = reading.RoomReadingImage
      .filter((img: any) => img.type === wallId)
      .map((img: any) => ({
        key: img.imageKey,
  ...
  return (
    <>
            ...
            <Stack mx="2" className="gap-y-2">
              ...
              <View>
                ...
                <RoomReadingInput
                  ...
                  onChange={(temperature) =>
                    updateRoomReading(reading.publicId, "standard", { temperature })
                  }
                />
              </View>
              ...
              <View>
                ...
                <RoomReadingInput
                  ...
                  rightText="gpp"
                  onChange={(gpp) =>
                    updateRoomReading(reading.publicId, "standard", { gpp })
                  }
                />
              </View>
              ...
              <View>
                <View className="flex-row items-center justify-between mb-0.5">
                  <View className="flex-row items-center">
                    ...
                    <TouchableOpacity onPress={() => handleAddExtendedWall("wall")} className="ml-2">
                ...
                {/* Wall Images */}
                {reading.RoomReadingImage && reading.RoomReadingImage.filter((img: any) => img.type === "wall").length > 0 && (
                  <View className="flex-row flex-wrap gap-1.5 mt-1 mb-1">
                    {reading.RoomReadingImage
                      .filter((img: any) => img.type === "wall")
              ...
              <View>
                <View className="flex-row items-center justify-between mb-0.5">
                  <View className="flex-row items-center">
                    ...
                    <TouchableOpacity onPress={() => handleAddExtendedWall("floor")} className="ml-2">
                      <Plus color="#1d4ed8" size={16} />
                    </TouchableOpacity>
                  </View>
                ...
                {/* Floor Images */}
                {reading.RoomReadingImage && reading.RoomReadingImage.filter((img: any) => img.type === "floor").length > 0 && (
                  <View className="flex-row flex-wrap gap-1.5 mt-1 mb-1">
                    {reading.RoomReadingImage
                      .filter((img: any) => img.type === "floor")
                      .map((img: any, index: number) => (
                        <Pressable 
                          key={img.imageKey}
              ...
              {reading.GenericRoomReading.map((grr: any, index: number) => (
                <GenericRoomReadingSection
                  ...
                  onImagePress={(imgIndex, genericId) => openImageViewer(imgIndex, 'generic', genericId)}
                />
              ))}
      ...
      <EditNameModal
        ...
        onChange={(name) => setCurrentEditingWall(prev => prev ? {...prev, name} : null)}
