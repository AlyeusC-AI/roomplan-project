...
const LidarScan = ({
  ...
  const handleStartScan = async () => {
    ...
    if (!processedRoomId.current) {
      ...
      const room = rooms.rooms.find(
        (room) => room.id === processedRoomId.current
      );
      if (room) {
  ...
  const handleScanComplete = async () => {
    try {
      ...
      const transformedJSON = await fetch(result.transformedRoomURL).then(
        (res) => res.json()
      );
  ...
  const confirmScanComplete = () => {
    ...
    Alert.alert("Finish Scan?", "Are you finished scanning this room?", [
      { text: "Continue Scanning", style: "cancel" },
      {
  ...
  const handleScanError = (ev: { nativeEvent: { message: string } }) => {
    ...
    Alert.alert("Error", "Failed to capture room. Please try again.", [
      { text: "OK", style: "cancel" },
    ]);
    setIsProcessing(false);
    setShowScanner(false);
    ...
    if (pngBase64.current) {
      ...
      try {
        ...
        const result = await new Promise((res) =>
          RoomScanModule.savePngFile(pngBase64.current, res)
        );
  ...
  if (isSupported === null) {
    ...
    return (
      <SafeAreaView className="flex-1 bg-white justify-center items-center px-5">
        ...
        <Text className="text-center text-base text-[#007AFF] mt-4 text-lg">
      ...
      <SafeAreaView className="flex-1 bg-white justify-center items-center px-5">
        {Platform.OS === "ios" ? (
          <>
            ...
            <Text className="text-center text-2xl font-bold mt-4 mb-2">
              LiDAR Sensor Required
            </Text>
            <Text className="text-center text-base leading-6 text-[#666]">
              This feature requires a device with a LiDAR sensor. Please use an
          ...
          <>
            ...
            <Text className="text-center text-base leading-6 text-[#666]">
              We're working hard to bring this feature to Android devices. Stay
              tuned for updates!
            </Text>
          </>
        )}
      ...
      <View className="relative inset-0 flex-1 bg-black">
        ...
        {!isScanProcessed && (
          <TouchableOpacity
            ...
            onPress={() => {
              finish !== -1 ? handleNextRoomScanStart() : confirmScanComplete();
            }}
          >
            <Ionicons
              name={finish === -1 ? "stop" : "scan-circle"}
              size={36}
              color="white"
            />
          </TouchableOpacity>
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <Text className="text-2xl font-bold mt-10 mb-5 text-center">
        3D Room Scanner
      </Text>
      <Text className="text-base text-center mx-5 mb-10 text-[#666]">
        Use your device's LiDAR sensor to create a 3D model of your room. This
        feature is only available on iOS devices with LiDAR sensors.
      </Text>
          ...
          >
            {imgkitLoading ? (
              ...
              <>
                ...
                <Text className="ml-1 text-base text-[#007AFF]">
                  Share Room Plan
                </Text>
              </>
            )}
      ...
      <View className="flex-row justify-around w-full mt-10">
        ...
        <TouchableOpacity
          ...
          disabled={!roomId && !newRoomName}
        >
          <Text className="text-white text-lg font-semibold">ESX</Text>
        </TouchableOpacity>
      </View>
      {showROomTypeSelect && (
        <LidarRoomTypeSelect
