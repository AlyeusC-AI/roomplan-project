...
const LocationData = ({
  ...
  useEffect(() => {
    ...
    const lngLat = {
      lat: parseFloat(projectInfo?.data?.lat ?? "0"),
      lng: parseFloat(projectInfo?.data?.lng ?? "0"),
    };
      ...
      {showWeather === true && (
        <div
          className={clsx(
            ...
            projectInfo?.data?.forecast?.toLowerCase() === "clouds" &&
              "from-blue-300 via-gray-400 to-gray-500",
            projectInfo?.data?.forecast?.toLowerCase() === "rain" &&
              "from-gray-400 via-gray-500 to-gray-700",
            projectInfo?.data?.forecast?.toLowerCase() === "clear" &&
          ...
          <div className='grid grid-cols-2 gap-y-2 bg-opacity-80 p-5 md:flex-col'>
            <div className='flex items-center'>
              <dl>
                ...
                <WeatherData>
                  {projectInfo?.data?.temperature ? (
                    <>
                      {Math.floor(parseFloat(projectInfo.data.temperature))}
            ...
            <div className='flex items-center'>
              <dl>
                ...
                <WeatherData>{projectInfo?.data?.forecast || "--"}</WeatherData>
              </dl>
            </div>
            <div className='flex items-center'>
              <dl>
