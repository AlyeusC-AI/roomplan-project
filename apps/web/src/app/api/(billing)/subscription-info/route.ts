...
export async function GET(request: NextRequest) {
  try {
    const [_, user] = await getUser(request);
    ...
    console.log("🚀 ~ GET ~ user:", user.user_metadata.organizationId);
    ...
    if (orgError) {
      console.log("🚀 ~ GET ~ orgError:", orgError);
      return NextResponse.json(
    ...
    const subscriptionInfo = {
      ...
      plan: subscription?.items.data[0]
        ? {
            ...
            features: features.data[0].marketing_features.map(
