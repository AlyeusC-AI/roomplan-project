...
export async function GET(
  ...
  try {
    ...
    let document;
    // Get document by ID
    const { data: documentIsExists, error: documentIsExistsError } =
...
//     document = documentIsExists;
//     if (!documentIsExists) {
//       const { data: originalDocument, error: originalDocumentError } =
//         await supabaseServiceRole
//           .from("Document")
//           .select("*")
//           .eq("publicId", id)
...
export async function PUT(
  ...
  try {
    ...
    const { data: newDocument, error: newDocumentError } =
      await supabaseServiceRole
        .from("Document")
        .update({
...
//     // Start a transaction
//     const { data: isDocumentExists, error: documentError } =
//       await supabaseServiceRole
//         .from("Document")
//         .select("*")
//         .eq("publicId", id)
...
//     if (isDocumentExists) {
//       const { data: updatedDocument, error: updatedDocumentError } =
//         await supabaseServiceRole
//           .from("Document")
//           .update({
//             json: validatedData.json,
