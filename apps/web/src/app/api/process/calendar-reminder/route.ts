...
export async function POST(req: NextRequest) {
  ...
  if (!calendarEvent) {
    return NextResponse.json({ status: 'ok', result: 'No action - Event not found.' }, { status: 200 })
  }
  ...
  if (!calendarEventReminder) {
    ...
    return NextResponse.json({ status: 'ok', result: 'No action - reminder not found.' }, { status: 200 })
  }

  if (calendarEvent.dynamicId !== dynamicId) {
    console.error('No action - reminder stale.')
    return NextResponse.json({ status: 'ok', result: 'No action - reminder stale.' }, { status: 200 })
  }
  // init nuvo
  ...
  try {
    ...
    return NextResponse.json({ status: 'ok', result: 'notification ran successfully' }, { status: 200 })
  } catch (error) {
